...
public function saveup(){
   ...
   $this->validate([
    'displayname'=>'required|regex:/(^([a-zA-z]+)(\d+)?$)/u|max:50',
    'color'=> 'required',
    'name'=> 'required|regex:/(^([a-zA-z]+)(\d+)?$)/u|max:50',
...
$orders=OrderStatusModel::where('id',$id)->first();
  ...
  $orders->save();
    ...
    {
      $search=$this->search.'%';
        $order =OrderStatusModel::where('display_name','LIKE',$search)->paginate(10);
        return view('livewire.super-admin.settings.order-status',['order'=>$order]);
    }
}
