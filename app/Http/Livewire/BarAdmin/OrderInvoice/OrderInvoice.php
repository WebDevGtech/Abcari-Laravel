...
class OrderInvoice extends Component
{
    ...
    public function render()
    {
        $search = '%' . $this->search . '%';
        $barRestaurant = BarRestaurant::where('user_id', Auth::id())->first();
        $this->orderInvoice = OrderInvoiceModel::with('orderTransaction', 'order', 'username', 'productName', 'orderInvoice')->where([['bar_restaurant_id', $barRestaurant->id], ['status', 'pending'],['order_no', 'LIKE', $search]])->paginate(10);
        return view('livewire.bar-admin.order-invoice.order-invoice', ['orders' => $this->orderInvoice]);
    }
}
