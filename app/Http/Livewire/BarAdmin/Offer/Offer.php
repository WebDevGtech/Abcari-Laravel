...
class Offer extends Component
{
    ...
    //  'image.image'=>trans('coupon.image_image'),
 ...
 $this->validate(['offer_id'=>'required'],['offer_id.required'=>trans('offer.offer_required')]);
        ...
        $addoffer->offer_type_id = $this->offer_id;
    ...
    public function editId($id)
    {
       ...
       // $this->edit_image=$edit->image;
        ...
        $this->edit_offer_id=$edit->offer_type_id;
...
$this->validate(['edit_offer_id'=>'required'],['edit_offer_id.required'=>trans('offer.offer_required')]);
 ...
 $barrestaurant=BarRestaurant::where('user_id',Auth::id())->first();
    ...
    $editoffer->offer_type_id=$this->edit_offer_id;
    ...
    'edit_image.max'=>trans('coupon.image_max'),]);
        $editimage=BarOffer::where('id',$editoffer->id)->first();
...
//dd($this->editoffertypes);
        $search=$this->search.'%';
        $barrestaurant=BarRestaurant::where('user_id',Auth::id())->first();
        $offer=BarOffer::with('category')->where([['bar_restaurant_id',$barrestaurant->id],['name','LIKE',$search]])->Paginate(15);
       // dd(auth::user()->countryname->currency_name);
        $category = Category::all();
        return view('livewire.bar-admin.offer.offer', ['offers' => $offer, 'offertypes' => $this->offertypes, 'editoffertypes' => $this->editoffertypes, 'categorys' => $category]);
    }
}
